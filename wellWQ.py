...
- Formulae available for calculation of Water quality indicators like Sodium Adsorption Ratio, Residual Sodium Carbonate, Soidum Percent, 
  Kelly's Ratio,Magnesium Hazard,Potential Salinity and Permeability index.
...
if menu in ["Descriptive Statistics", "Visualizations", "Correlation Analysis"]:
    ...
    if menu == "Descriptive Statistics":
        ...
        stat = st.sidebar.multiselect("Select Statistics", ["Mean","Median","Minimum","Maximum","Standard deviation"])
        if stat: st.dataframe(filtered.groupby(["Year","Season"])[param].agg(stat).reset_index())
